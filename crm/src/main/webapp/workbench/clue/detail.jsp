...
<script type="text/javascript">
	...
	$(function(){
		...
		//在页面加载完毕之后取出关联的市场活动列表
		 showActivityList();

		//为关联市场活动模态窗口中的搜索框 绑定事件 通过触发回车键 查询并展现
		$("#aname").keydown(function (event) {
			...
			if ($xz.length==0){
				...
				//要传递的参数：
					//workbench/clue/bund.do?cid=xxx&aid=xxx&aid=xxx&aid=xxx(可能关联多个市场活动)
				var param="cid=${c.id}&";
			...
			success:function (data) {
				...
				* */
				...
				$.each(data,function (i,n) {
					...
					html += '<font color="gray">线索</font> <font color="gray">-</font> <b>${a.name}</b> <small style="color: gray;"id="s'+n.id+'"> '+(n.editFlag==0?n.createTime:n.editTime)+' 由'+(n.editFlag==0?n.createBy:n.editBy)+'</small>';
	...
	//解除市场活动关联
	//这个参数id 是市场活动id 同样线索的id 是 $ {c.id} 把这两个id结合 传递到数据库 作为条件就可以删除线索对应的市场活动
	//更好的办法：把这个参数id变成关系表的id 可以根据id删单条
	function unbund(id) {
		if(confirm("您确定要解除该市场活动的关联吗?")){
			$.ajax({
